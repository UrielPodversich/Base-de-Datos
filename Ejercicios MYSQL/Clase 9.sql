...
SELECT
	...
	FROM
		rental r
	WHERE
		c.customer_id = r.rental_id) AS total_rented,
	(
	SELECT
		SUM(p.amount)
	FROM
		payment p
	WHERE
		c.customer_id = p.customer_id) AS total_spent
FROM
	customer c
JOIN address a ON
...
SELECT
	c.name,
	AVG(f.`length`) AS average_lenght_category
FROM
	film f
